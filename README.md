...
- setup a cast app on https://cast.google.com/ using `ngrok` to serve an https site running on your dev machine
  - Your cast app can be anything but to debug a streaming site's app, you could load their cast app assuming you can find their cast_app_id (possibly by searching decompiles apk or looking in the file system from their apk modified to enable debugging in the manifest)
  - Note that you could use something like ws in a little node app like we do for site-scrapers to host your cast-receiver site.
...
- to change the cast_app_id run a command like `frida -U -f com.netflix.mediaclient -l ./patch-cast-app-id.js`


# Attempt to debug Netflix casting
My first goal was to identify the netflix ESN that was being used by the cast-receiver device. I discovered that the netflix cast app was exceedingly complex. Note that this only really applies to older cast devices (generation 1 & 2) since casting to an android TV device simply forces the launching of the netflix app on that device.

## Special Handling Between Netflix & Google
The main issues I ran into is that it seems there were special deals setup between Netflix and Google in the early days of casting that are difficult to spoof.
...
```
Note: Something I considered but didn't try was to use my EdgeMax router to re-route the url above to my own server that could respond and include this special prop. However, I surmized that it would require https/SSL and I wasn't sure that a self-signed certificate would be accepted. My SSL and cert skills are not expert level, but if we could do that we might be able to see more of what I suspect these extended features do... keep reading.
...
It was at this point I decided to take a break and move on for now. The remaining ideas I had were to try to re-route the cast url for the netflix cast app id using my Edgemax Router, and to see if it would be possible to root an older generation 1 or 2 chromecast device to gain access to the file system to look for the device's DKE key (perhaps even run frida on it). However, some quick google searching didn't reveal any current known rooting exploits for these devices.